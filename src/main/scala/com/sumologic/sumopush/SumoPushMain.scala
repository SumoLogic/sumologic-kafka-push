    ...
    Behaviors.setup[NotUsed] { context =>
      ...
      val consumer = config.dataType match {
        ...
        case SumoDataType.metrics =>
          ...
          context.spawn(PushConsumer(config, Consumer.committableSource(settings, Subscriptions.topics(config.topic)), context => MetricsFlow(context, config, k8sMetadataCache)), "metric-consumer")
      }
